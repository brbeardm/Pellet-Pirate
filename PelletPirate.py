...
u_max = 1.0  # flymanedit no comment really just like comments
...
logger = logging.getLogger('PelletPirate')


#Initialize RTD Probes
T = []
T.append(MAX31865.MAX31865(0, 1000, 4000, False))  # Grill
T.append(MAX31865.MAX31865(1, 100, 400, True))  # Meat1
#T.append(MAX31865.MAX31865(2, 1000, 4000, False))  # Meat2


#Initialize Traeger Object
G = Traeger.Traeger(Relays)

# PID controller based on proportional band in standard PID form from
...
#Start firebase
f = open('/home/pi/pelletpirate/pelletpirate/AuthToken.txt', 'r')
Secret = f.read()
f.close()
Params = {'print': 'silent'}
...
#logger.info(Secret)
#logger.info(Params)
firebase = firebase.FirebaseApplication('https://pellet-pirate.firebaseio.com/')
...
def PostTemps(Parameters, Ts):
    try:
        ...
        logger.info('Success!, just wrote Temps to Firebase from PostTemps()')
    except:
        logger.info('Error PostTemps() writing Temps to Firebase')
...
def WriteParameters(Parameters):
    ...
    for r in Relays:
        ...
        qP.put(Parameters)
    try:
        r = firebase.patch_async('/Parameters',
            Parameters, params=Params, callback=PostCallback)
    except:
...
def ReadParameters(Parameters, Temps, Program):
    ...
    if time.time() - Parameters['LastReadWeb'] > ReadParametersInterval:
        ...
        try:
            ...
            logger.info('Success!, just read Parameters from Firebase as NewParameters')
            #logger.info('New parameters from firebase: %s', NewParameters)
            (Parameters, Program) = UpdateParameters(NewParameters, Parameters, Temps, Program)
        except:
            logger.info('Error ReadParameters() reading parameters from Firebase')
...
# flymanedit - renamed sum to suma cause sum is a function in python
    n = 0
    suma = [0] * len(Temps[0])
    for Ts in Temps:
        if Ts[0] < startTime:
...
def GetProgram(Parameters, Program):
    if time.time() - Parameters['LastReadProgram'] > ReadProgramInterval and Parameters['program']:
        try:
            logger.info('in GetProgram try line 357')
...
while 1:
    # Record temperatures
    logger.info('MAIN LOOP before calling RecordTemps')
    Temps = RecordTemps(Parameters, Temps)

    # Check for new parameters
    logger.info('MAIN LOOP before calling ReadParameters')
    logger.info('Program: ' + str(Program))
    (Parameters, Program) = ReadParameters(Parameters, Temps, Program)

    # Check for new program
    logger.info('MAIN LOOP before calling GetProgram')
    logger.info('before GetProgram() Program: ' + str(Program))
    Program = GetProgram(Parameters, Program)
    logger.info('after GetProgram() Program: ' + str(Program))
    # Evaluate triggers
    logger.info('MAIN LOOP before calling EvaluateTriggers')
    (Parameters, Program) = EvaluateTriggers(Parameters, Temps, Program)

    # Do mode
    logger.info('MAIN LOOP before calling DoMode')
